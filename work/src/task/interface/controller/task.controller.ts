...
export class TaskController {
  ...
  async updateTask(
    ...
    const command = new UpdateTaskCommand(taskId, req.user.id, { ...data });
  ...
  async updateDueDate(
    ...
    const command = new UpdateDueDateCommand(taskId, req.user.id, data.dueDate);
  ...
  async deleteTask(@Req() req: RequestWithUser, @Param('id') taskId: string) {
    const command = new DeleteTaskCommand(taskId, req.user.id);
    const result: Result<IdResponse, TaskNotFoundError> =
      await this.commandBus.execute(command);
    return match(result, {
      Ok: (id) => id,
  ...
  async addTaskMember(
    ...
    const command = new AddTaskMemberCommand(
      taskId,
      req.user.id,
      data.memberId,
    );
  ...
  async addTaskLabel(
    ...
    const command = new AddTaskLabelCommand(taskId, req.user.id, data.labelId);
    const result: Result<
      IdResponse,
  ...
  async removeTaskLabel(
    ...
    const command = new RemoveTaskLabelCommand(
      taskId,
      req.user.id,
      data.labelId,
    );
  ...
  async getBoardHistory(@Req() req: RequestWithUser, @Param('id') id: string) {
    ...
    const result: Result<History[], TaskNotFoundError> =
      await this.queryBus.execute(query);
    return match(result, {
      Ok: (histories) => histories.map((history) => history.changedContent),
      Err: (err) => {
        if (err instanceof TaskNotFoundError)
